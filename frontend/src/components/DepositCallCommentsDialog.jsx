...
const DepositCallCommentsDialog = ({
  open,
  onClose,
  depositCall,
  onCommentAdded,
  ...
  useEffect(() => {
    if (open && depositCall?._id) {
      fetchComments();
    }
  }, [open, depositCall?._id]);
  ...
  const fetchComments = async () => {
    if (!depositCall?._id) return;
    try {
      setLoading(true);
      setError(null);
      const response = await depositCallsService.getComments(depositCall._id);
      if (response.success) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      setError(null);
      await depositCallsService.addComment(depositCall._id, newComment.trim());
      ...
      <DialogTitle sx={{ pb: 1 }}>
        ...
        {depositCall && (
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            {depositCall.ftdName || `${depositCall.leadId?.firstName || ''} ${depositCall.leadId?.lastName || ''}`.trim()}
            {depositCall.ftdEmail ? ` - ${depositCall.ftdEmail}` : ''}
          </Typography>
        )}
      </DialogTitle>
...
export default DepositCallCommentsDialog;
